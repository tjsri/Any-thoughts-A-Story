...
// UI Elements
let saveButton, flyButton, homeButton, recordButton; 
...
function draw() {
  ...
  if (isRecording) {
    fill(255, 0, 0);
    noStroke();
    circle(windowWidth - 30, 80, 15);
  }
}

function spawnLetters(x, y) {
  // Loop the story back to the start if we hit the end
...
function setupUI() {
  ...
  flyButton = createButton('RELEASE');
  ...
  recordButton = createButton('REC VIDEO');
...
function repositionUI() {
  ...
  flyButton.position(400, 20);
  saveButton.position(510, 20);
  recordButton.position(625, 20);
...
function setupRecorder() {
  ...
  mediaRecorder.onstop = () => {
    ...
    a.href = url;
    a.download = 'myStory_Interactive.webm';
    a.click();
    recordedChunks = [];
  };
}
